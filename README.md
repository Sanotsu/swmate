...
- 支持添加非预设云平台但兼容 OpenAI API 结构的 API
  - 比如 [chutes.ai](https://chutes.ai/)、[openrouter.ai](https://openrouter.ai/models)
- 页面适配了桌面端和移动端，并测试和构建了 Android、Windows、Linux 版本应用可使用
- 合并了分支对话和角色扮演
- 更完善的功能: AI 对话、AI 生图、AI 视频、语音合成、录音文件识别、简单的角色扮演等
- 修正大量问题，更丰富的细节改善
...
- 以简单参数调用各个云平台部署的在线大模型 API 为基底的“**AI 智能助手**”，(简称“工具”)。
  ...
  - 对话模型：
    ...
    - 2025-05-14 在模型管理中可手动添加非预设云平台(比如 OpenRouter 等)的模型
      - 平台选择自定义，再指定模型分类，添加模型名、baseUrl、APIKey 即可使用
  - 图片生成：
...
该平台有很多不同的大模型 API 可以调用，比如最新的 DeepSeek、Qwen、GLM 等。
...
2025-05-14 自定义云平台要额外添加的`baseUrl`,`apiKey`栏位。
