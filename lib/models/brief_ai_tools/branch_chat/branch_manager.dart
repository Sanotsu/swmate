...
class BranchManager {
    ...
    for (var message in sortedMessages) {
      ...
      if (messagePath.length == pathIndices.length + 1 &&
          messagePath.sublist(0, pathIndices.length).join('/') ==
              pathIndices.join('/')) {
        final parentPath =
            messagePath.sublist(0, messagePath.length - 1).join('/');
        final currentIndex = int.parse(messagePath.last);
        maxChildIndices[parentPath] =
            max(maxChildIndices[parentPath] ?? 0, currentIndex);
      }
    }
  ...
  /// 获取消息的分支索引
  int getBranchIndex(
      List<BranchChatMessage> messages, BranchChatMessage message) {
  ...
  /// 获取下一个可用的分支索引
  int getNextAvailableBranchIndex(List<BranchChatMessage> messages,
      BranchChatMessage message, int targetIndex) {
    final siblings = getSiblingBranches(messages, message);
